...
def http_get(url, source, retries=2, timeout=20, headers_extra=None,
    ...
    # Live fetch failed entirely — serve last-good copy if permitted.
    if allow_stale:
...
tab_watch, tab_terminal = st.tabs(
    ["📋 Market Watch", "🏢 Company Terminal"])
...
try:
    ...
    mw = None
    st.error(f"⚠️ Official PSX market data is unavailable right now: {e}. "
             "No simulated data is shown by design.")
    st.stop()

syms = get_symbols()
SYMBOL_INFO = {s["symbol"].upper(): s for s in syms}
...
with tab_watch:
    ...
    df = mw.merge(pd.DataFrame(syms)[["symbol", "name", "sectorName"]]
...
def _terminal():
    all_syms = sorted(SYMBOL_INFO.keys())
    ...
    verdict3, why3, colour3 = risk_verdict(composite3)

    # ---- headline verdict ----
    st.markdown(
        f"<div style='padding:18px;border-radius:12px;"
        f"background:{colour3}22;border:2px solid {colour3};'>"
        f"<span style='font-size:30px;font-weight:800;color:{colour3};'>"
        f"{verdict3}</span>"
        f"<span style='float:right;font-size:26px;font-weight:700;"
        f"color:{colour3};'>{composite3:+.1f} / ±100</span>"
        f"<div style='color:inherit;margin-top:6px;'>{why3}</div></div>",
        unsafe_allow_html=True)
    ...
    issues, checked, passed = run_integrity_checks(mw, syms)
    okr, wr = st.columns(2)
